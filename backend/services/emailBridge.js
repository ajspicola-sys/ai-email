...
async function processEmployeeInbox(employeeEmail, accessToken, activeRules) {
  // Only look at unread emails received in the last 1 hour (to prevent back-processing thousands of old unread emails!)
  const oneHourAgo = new Date(Date.now() - 3600000).toISOString();
  const filter = encodeURIComponent(`isRead eq false and receivedDateTime ge ${oneHourAgo}`);
